...
function getArray(list) {
	return list.map(function(item) { return [item['Kode'], item['harga'].replace(".", "")]; });
}
	...
	createHarga: (json, args, callback) => {
		...
		async.forEachOf(array_json, function(nilai, iterasi, callback) {
			...
			async.forEachOf(operator, function(value, key, callback) {
				...
				var kode = value[0],
					harga = parseInt(value[1].replace(".", "")),
			...
			if (err) {
				...
				callback(null, 'Sukses Simpan dari web Portal Pulsa')
			}
		})
	},
};